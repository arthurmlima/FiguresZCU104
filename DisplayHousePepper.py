...
def display_four_images(image_paths, fb_file='/dev/fb0', margin_percent=5):
    try:
        ...
        for i, image_path in enumerate(image_paths):
            with Image.open(image_path) as img:
                ...
                # Calculate position to center the image in its quadrant
                x_offset = horizontal_margin + (i % 2) * (available_width + horizontal_margin)
                y_offset = vertical_margin + (i // 2) * (available_height + vertical_margin)
        ...
        print(f"Four images displayed in a 2x2 grid on {fb_file}")
...
if __name__ == "__main__":
    ...
    # Define the four image paths here
    image_paths = [
    ...
    print("Four images displayed in a 2x2 grid on HDMI monitor. Press Ctrl+C to exit.")
