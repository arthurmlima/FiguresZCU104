...
def display_two_images(image_paths, fb_file='/dev/fb0', margin_percent=5):
    try:
        ...
        available_width = (var_info.xres - 3 * horizontal_margin) // 2
        available_height = var_info.yres - 2 * vertical_margin
        ...
        for i, image_path in enumerate(image_paths):
            with Image.open(image_path) as img:
                ...
                # Calculate position to center the image in its half
                x_offset = horizontal_margin + (i * (available_width + horizontal_margin))
                x_offset += (available_width - new_size[0]) // 2
                y_offset = vertical_margin + (available_height - new_size[1]) // 2
        ...
        print(f"Two images displayed side by side on {fb_file}")
...
if __name__ == "__main__":
    ...
    # Define exactly two image paths here
    image_paths = [
        os.path.join(script_dir, "Encrypted/plain_Pepper.jpg"),
        os.path.join(script_dir, "Encrypted/Pepper.jpg"),
    ]
    ...
    display_two_images(image_paths)
    print("Two images displayed side by side on HDMI monitor. Press Ctrl+C to exit.")
